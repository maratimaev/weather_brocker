 ...
 */
@Stateless
public class YahooPollerImpl implements YahooPoller {
    /**
    ...
    /**
     * доступ к методам REST Services
     */
    ...
    public String getWeatherFromYahoo(String city) {
        ...
        HttpEntity httpEntity = new HttpEntity(createHeaders(authorizationLine));
        ResponseEntity<String> response = restTemplate.exchange(
                URL + "?location=" + city + "&format=json",
                HttpMethod.GET,
                httpEntity,
                String.class
        );
        if(response == null){
            throw new RuntimeException("(Custom) Error -> no data recieved from yahoo.com");
        }
